...
有两个模板参数，Key 表示数据类型, Comparator 是数据对应的比较器类型；   
memtable 存储的是 memtable key，它是字符串，所以 Key 被指定为 const char*。相应地，Comparator 被指定为 KeyComparator，它包含一个InternalKeyComparator 成员，用于比较 internal key。
