...
从图2可以看出，跳表的查找次数等于 H * k（每一层最多需遍历的节点数，视作一个常数因子），所以时间复杂度表示为 O(klog n)，而 k 是常数，可忽略，最终表示为 O(log n)，这跟平衡二叉/多叉树是一致的。      

**节点分布**   
节点分布规律：分支为2 时，对于达到层级 i 的节点，其中有一半会达到层级 i+1，很显然，不同层级节点分布满足：层级1 占比50%，层级2 占比25%，层级3 占比12.5%，依次类推。   
**注：** 层级k 的节点是满足高度 h=k 的节点，而达到层级k 的节点则是满足 h >= k 的所有节点。    
...
### PS
1、对于各层级节点概率的推理：   
设节点从层级 i 到层级 i+1 的晋升概率是1/b，易得 P(h>=i+1) = P(h>=i) * 1/b；   
已知 P(i>=1) = 1，递推可得 P(i>=i) = 1/b<sup>i-1</sup>；    
当达到层级 i 的节点不晋升时，便可得到第 i 级节点，不晋升的概率是 1-1/b；    
而节点达到到层级 i 的概率是 P(i>=i)，可得 P(h=i) = P(h>=i) * (1-1/b)；
则高度概率分布 P(h=i) = (1-1/b)/b<sup>i-1</sup>。   

2、时间与空间的权衡      
设跳表的的分支数为 2<sup>i+1</sup> (i >= 0)，高度为 h，节点数 n。   
当分支等于 2 时，h<sub>1</sub> = log<sub>2</sub>n；   
当分支大于 2 时，h<sub>2</sub> = log<sub>2<sup>i+1</sup></sub>n (i >= 1)；   
可知，2<sup>h<sub>2</sub>(i+1)</sup> = 2<sup>h<sub>1</sub></sup>，即 h<sub>2</sub>(i+1)</sup> = h<sub>1</sub>。   
跳表查找次数为 c = k * h，那么，
当分支等于 2 时，c<sub>1</sub> = 2h<sub>1</sub> = 2(i+1)h<sub>2</sub>；   
当分支大于 2 时，c<sub>2</sub> = 2<sup>i+1</sup>h<sub>2</sub>；   
显然，当 i >= 1 时，2(i+1) <= 2<sup>i+1</sup>，即 c<sub>1</sub> <= c<sub>2</sub>；    
当且仅当 i=1 时，即分支为4时，c<sub>1</sub> = c<sub>2</sub> 成立。   

通过上述论证可知，在理想情况下，跳表在分支4和分支2的时间复杂度是一样的，不过由于概率机制的存在，4分支跳表在理论上会稍逊于2分支跳表。因此，实际使用时往往会将跳表分支设置为4，可以在更低空间占用下取得媲美2分支跳表的读写性能。
