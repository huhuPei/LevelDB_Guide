...
**时间复杂度**     
假设跳表节点数为 n，跳表高度为 H，分支数为2，那么 height >= 2 的节点数是 n / 2，height >= 3 的节点数是 n / 4，以此类推，height >= H 的节点数是 1 = n / 2^H-1^，跳表高度 H = $\log_2 n$ + 1。    
...
因此，在实现跳表结构时，节点的高度采用随机函数生成的方式，给定一个分支数 b，使得达到层级 i 的节点晋升到层级 i+1 的概率为 p = 1/b，即 P(h>=i) = 1/b^i-1^，那么生成 i 级节点的概率为 P(h=i) = P(h>=i) * (1-1/b) = (1-1/b) / b^i-1^。   

**概率分布**     
设跳表分支为2，晋升概率 p = 1/2，P(h=i) = 1/2^i^，那么各个层级节点的概率如下：    
P(h=1) = 1/2，   
P(h=2) = 1/4，   
...
P(h=k) = 1/2^k-1^。   
...
#### 高度限制
**合理性**   
1、为了节省内存空间和维持良好性能，实际跳表的高度不会无限增长。如果当前节点数量较少，高度过高不但起不到加快查询速度的作用，反而还会影响查询速度，同时还会浪费内存空间。比如现在节点数 n 为1024，那么跳表的理想高度是 $\log_2 n$ = 10，然而节点的高度是随机的，因此会有部分节点的高度超过10，随着 n 的增大，这种过高节点会更多，过高的高度增加查询的比较次数，导致跳表性能下降，因此过高高度分配的指针空间也是无效的。   
...
2、compare_ 是一个 InternalKeyComparator；  
...
```
#### 随机高度方法
levelDB 选择晋升概率 1/4 而不是 1/2，是为了在时间复杂度和空间复杂度之间找到一个较好的平衡点。与常用的概率 1/2 相比，1/4 的概率会使跳表的高度增长更慢，往上晋升的节点也更少，从而减少空间开销，同时仍能保持相对较低的查找时间复杂度。理想情况下，概率为 1/4 时，跳表的高度为 $\log_4 n$，而概率为 1/2 时高度为 $\log_2 n$。显然，随着数据量的增加，$\log_4 n$ 增长更慢，空间占用更少。
```
// db/skiplist.h
...
bool SkipList::KeyIsAfterNode(const Key& key, Node* n) const {
  // NULL 必返回 false；调用 InternalKeyComparator 比较器
...
现在用户使用序列号 8，读取 key1 的值，内部会返回 {uk: key1, s: 7, v: 1}，那么读取到的值为 1，在这之后的更新版本 {3, 2} 会被隔离，即用户只能读取到序列号 8之前的最近的数据。    
...
已知 P(i>=1) = 1，递推可得 P(i>=i) = 1/b^i-1^；    
当达到层级 i 的节点留在层级i 时，便可以得到 i 级节点，上升的概率是1/b，那么留下的概率便是 1-1/b；    
而节点达到层级i 的概率是 P(i>=i)，可得 P(h=i) = P(h>=i) * (1-1/b)；
则高度概率分布 P(h=i) = (1-1/b)/b^i-1^。   

关于时间复杂度与晋升概率的关系          


论文链接：https://15721.courses.cs.cmu.edu/spring2018/papers/08-oltpindexes1/pugh-skiplists-cacm1990.pdf