...
#### internal key    
序列号和值类型组成一个 tag，每个 user key 都会添加一个 tag，组成 internal key。tag 一共占用8bytes，sequence number 占7bytes，value type 占1bytes。

**为什么需要对 user key 添加 tag 字段呢？**       
用户每次 put/delete 数据时，数据库内部都会分配一个序列号，这个序列号是严格递增的，用于表示数据的新旧程度。同一个 user key 可能对应着多个不同版本的 value，通过序列号进行区分，序列号越大，表示 value 越新。同时，为了区分出 delete，加入了 type 字段，值为0时，表示 key 已删除，值为1时，表示 key 存在。因此，tag 的作用就是用于标识数据版本和操作类型。     

从这里可以看出，用户对数据进行更新/删除操作，在底层都是插入操作，每个 user key 都会被封装为 internal key 插入到 memtable 中。

**序列号作用**   
相当于版本信息，表示数据的新旧程度，用于实现版本控制，保证并发下的数据一致性。
快照隔离：默认情况下，get 操作会返回 user key 的最新版本，也就是序列号最大的 value。如果需要读取某一版本的数据，就要进行 snapshot 操作，通过指定一个序列号s，序列号小于等于 s 的数据可以被读取，序列号大于 s 的数据将被隔离，从而实现读取特定版本的数据。
